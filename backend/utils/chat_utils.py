...
def transform_stlit_to_genai_history(
    transform_history, is_entire_code_loaded, entire_code
):
    genai_history = []
    for message in transform_history:
        role = "user" if message["role"] == "user" else "model"
        genai_history.append({"role": role, "parts": [{"text": message["content"]}]})
